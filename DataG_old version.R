simulate_data_U1 <- function(n, tau) {
...
simulate_data_U3 <- function(n, tau) {
  
  ## ---- Generate baseline covariates & unmeasured confounder ----
  X1 <- rbinom(n, size = 1, prob = 0.4)    # X1 ~ Bern(0.4)
  X2 <- rbinom(n, size = 1, prob = 0.6)    # X2 ~ Bern(0.6)
  U3 <- rgamma(n, shape = 0.5, scale = 0.5)  # U3 ~ Gamma(0.5, 0.5)
  ...
  linpred_A <- alpha_0 + alpha_x1 * X1 + alpha_x2 * X2 + alpha_u * U3
  pi_A      <- 1 / (1 + exp(-linpred_A))
  A         <- rbinom(n, size = 1, prob = pi_A)
  ...
  lp_a1   <- eta_intercept_a1 + eta_x1 * X1 + eta_x2 * X2 + eta_u * U3
  lp_a0   <- eta_intercept_a0 + eta_x1 * X1 + eta_x2 * X2 + eta_u * U3
