...
for (i in 1:n_sim) {
  ...
  spce_estimates_EM_aft[i] <- res_spce_weight_aft$SPCE
  spce_estimates_EM_cox[i] <- res_spce_weight_cox$SPCE
  
  spce_estimates_Sto_aft[i] <- stoEM_SPCE_W_aft$SPCE
  spce_estimates_Sto_cox[i] <- stoEM_SPCE_W_cox$SPCE
  
  ## --------------------------------------- ##
  ## === BOOTSTRAP inside the iteration  === ##
  ## --------------------------------------- ##
  B_boot <- 100
  boot_spce_EM_aft  <- boot_spce_EM_cox  <-  boot_spce_Sto_aft <- boot_spce_Sto_cox <-  numeric(B_boot)
...
results_df <- data.frame(
  EM_AFT        = spce_estimates_EM_aft,
  EM_Cox        = spce_estimates_EM_cox,
  StoEM_AFT     = spce_estimates_Sto_aft,
  StoEM_Cox     = spce_estimates_Sto_cox,
  EM_AFT_SD     = sd_EM_aft,
  EM_Cox_SD     = sd_EM_cox,
  StoEM_AFT_SD  = sd_Sto_aft,
  StoEM_Cox_SD  = sd_Sto_cox
)
